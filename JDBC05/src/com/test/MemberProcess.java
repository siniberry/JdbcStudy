...
// .toString -> Object 클래스가 가진 메소드로 객체가 가지고 있는 정보나 값들을 문자열로 만들어 리턴하는 메소드
// .append(city+"/" ); // - > cityStr의 요소의 맨마지막에 표시하고자 하는 도시/ 를 붙여줌
// ArrayList<> 가변적으로 변하는 리스트 
// printf / String.format -> prinf 는 화면상으로 출력할 때, String.fotmat은 넘겨줄때 형식넘겨주는거임
// StringBuilder -> https://ifuwanna.tistory.com/221 참고 
// 문자열 연산이 많고 단일쓰레드이거나 동기화를 고려하지 않아도 되는 경우  주로 사용하며
// 
	...
	public void memberInsert()
	{
		...
		try
		{
			...
			for (String city : citys)
				cityStr.append(city+"/" ); // - > cityStr의 요소의 맨마지막에 표시하고자 하는 도시/ 를 붙여줌
	...
	public void memberUpdate()
	{
		...
		try
		{
			...
			if (memList.size() > 0)
			{
				...
				System.out.printf("수정 지역(%s) : ",cityStr.toString());  // 그냥 cityStr로 받아도 OK
				String cityName = sc.next();
				if (cityName.equals("-"))
					cityName = mCityName;
